...
export default function MovieCard({ movie, onClick }: MovieCardProps) {
  ...
  return (
    <motion.div
      ...
      onClick={onClick}
      className="group relative aspect-[2/3] rounded-lg overflow-hidden cursor-pointer transform transition-all duration-300 hover:scale-105 hover:shadow-2xl hover:shadow-[#696969]/20 bg-gradient-to-br from-[#232323] to-[#000000] border border-[#343434]/30"
    >
      {/* Movie Poster or Placeholder */}
      <div className="relative w-full h-full bg-gradient-to-br from-[#232323] via-[#000000] to-[#343434]">
        {hasPoster ? (
          <motion.div layoutId={`image-${movie.id}`} className="relative w-full h-full">
            <Image
              src={posterUrl}
              alt={movie.title}
              fill
        ...
        {(movie.voteAverage != null && !isNaN(movie.voteAverage) && movie.voteAverage > 0) && (
          <motion.div 
            layoutId={`rating-${movie.id}`}
            className="absolute top-3 right-3 bg-gradient-to-r from-[#ffc300] to-[#ffd60a] backdrop-blur-sm text-[#000814] px-3 py-1.5 rounded-full flex items-center gap-1.5 font-bold text-sm z-10 shadow-lg shadow-[#ffc300]/30"
          >
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 transform translate-y-full group-hover:translate-y-0 transition-transform duration-300">
          <motion.h3 
            ...
            className="text-white font-bold text-lg mb-2 line-clamp-2 group-hover:text-[#ffc300] transition-colors duration-300"
          >
            {movie.title}
          </motion.h3>
